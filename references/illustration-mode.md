...
このファイルは通常の挿絵を対象とする。元画像を**文字ごと新しいスライド用画像に再生成**する場合は [`grammar-image-to-click-workflow.md`](grammar-image-to-click-workflow.md)、元画像がなく**内容から2K図解画像を起こす**場合は [`content-to-2k-image-click-workflow.md`](content-to-2k-image-click-workflow.md) を使う。以下の「重要文字を画像生成へ書かせない」規則は通常の補助挿絵に適用し続ける。
