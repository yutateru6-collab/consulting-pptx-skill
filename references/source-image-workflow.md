...
5. manifestを作り、`python3 scripts/check_source_images.py deck.pptx image-manifest.json` を実行する。Pillowが未導入なら `python3 -m pip install Pillow`。検査は切り抜きの画素一致、再生成PNGを最後まで読み込めること、そのPNGの埋め込み、説明パネルが各内容スライドの面積の30%以上を占めること、表示枠に対する画像画素数を確認する。
