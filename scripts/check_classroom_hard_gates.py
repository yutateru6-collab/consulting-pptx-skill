...
import argparse, json, posixpath, re, zipfile
...
def sizes(sh):
    return [r.font.size.pt for p in sh.text_frame.paragraphs for r in p.runs
            if r.text.strip() and r.font.size] if getattr(sh,"has_text_frame",False) else []

def minpt(sh):
    s=sizes(sh); return min(s) if s else None

def maxpt(sh):
    s=sizes(sh); return max(s) if s else None
...
def choose_title(slide, cover=False):
    ...
    for sh in slide.shapes:
        s=txt(sh).strip()
        if not s: continue
        top=sh.top/EMU; width=sh.width/EMU; size=maxpt(sh) or 0
        if cover:
            if top<=4.5 and width>=3.0: cs.append((-size,top,sh))
        else:
            if top<=1.6 and width>=4.5 and size>=20: cs.append((top,-size,sh))
    return min(cs, default=(0,0,None))[2]

def notes_text(zf, n):
    rel=f"ppt/slides/_rels/slide{n}.xml.rels"
    if rel not in zf.namelist(): return ""
    rr=ET.fromstring(zf.read(rel))
    target=None
    for r in rr:
        if r.get("Type","").endswith("/notesSlide"):
            target=r.get("Target"); break
    if not target: return ""
    part=posixpath.normpath(posixpath.join("ppt/slides",target))
    if part not in zf.namelist(): return ""
    root=ET.fromstring(zf.read(part))
    s=" ".join((e.text or "").strip() for e in root.findall(".//a:t",NS) if (e.text or "").strip())
    s=re.sub(r"\b\d+\b"," ",s)
    return re.sub(r"\s+"," ",s).strip()

def click_count(zf,n):
    part=f"ppt/slides/slide{n}.xml"
    if part not in zf.namelist(): return 0, False, False
    root=ET.fromstring(zf.read(part))
...
def profile_for(path,prs,requested):
    ...
    if prs.slides: probe+=" "+ " ".join(txt(s) for s in prs.slides[0].shapes).lower()
...
def check(path,profile="auto",allow_static=False,notes_optional=False):
    ...
    total_clicks=0; note_count=0; click_slides=0
    with zipfile.ZipFile(path) as zf:
        for i,slide in enumerate(prs.slides,1):
            ...
            for sh in slide.shapes:
                ...
                if short_label(sh,s):
                    if p<LABEL_MIN: fails.append(f"p{i}: label {p:.1f}pt < {LABEL_MIN:.0f}pt: {s[:60]!r}")
                elif p<BODY_MIN:
            ...
            c,timing,auto=click_count(zf,i); rec["clicks"]=c; total_clicks+=c
            if c: click_slides+=1
            if auto: fails.append(f"p{i}: automatic slide advance is enabled")
            if i!=1 and not allow_static:
                if c==0: fails.append(f"p{i}: no on-click reveal effects")
                if question_slide(slide) and c<2: fails.append(f"p{i}: exercise/quiz slide needs >=2 click effects")
            slides.append(rec)

    if len(prs.slides)>1 and not notes_optional:
        content=len(prs.slides)-1
        n=sum(1 for r in slides[1:] if r["notes"])
        if n<content: fails.append(f"notes coverage {n}/{content}; expected every content slide")
    if len(prs.slides)>1 and not allow_static:
        content=len(prs.slides)-1
        c=sum(1 for r in slides[1:] if r["clicks"]>0)
        if total_clicks==0: fails.append("deck-level animation gate: 0 click effects")
        if c<content: fails.append(f"click coverage {c}/{content}; expected every content slide")
    ...
    if prof=="flowchart":
        ...
        for i,sl in enumerate(prs.slides,1):
            ...
            if lines>=3 and auto>=4: flow.append(i)
            if auto>=8 and lines<=1 and textblocks>=8: cards.append(i)
        if len(prs.slides)>=8 and len(flow)<2:
            fails.append("flowchart profile: fewer than two structurally flow-like slides")
        if len(cards)>=3:
            fails.append(f"flowchart profile: repeated card-grid slides {cards[:10]}; use a persistent decision spine/branch zoom")

    return {
        "file":str(path),"slides":len(prs.slides),"profile":prof,
        "fail_count":len(fails),"warn_count":len(warns),
        "fails":fails,"warnings":warns,"slide_summary":slides,
        "summary":{"total_click_effects":total_clicks,"slides_with_clicks":click_slides,"notes_present":note_count}
    }
...
def main():
    ...
    if not a.pptx.exists() or a.pptx.suffix.lower()!=".pptx":
        print(f"FAIL: invalid PPTX path: {a.pptx}"); return 2
    r=check(a.pptx,a.profile,a.allow_static,a.notes_optional)
    for x in r["fails"]: print("FAIL:",x)
    for x in r["warnings"]: print("WARN:",x)
    print(f"HARD_GATES: profile={r['profile']} slides={r['slides']} FAIL={r['fail_count']} WARN={r['warn_count']} clicks={r['summary']['total_click_effects']} notes={r['summary']['notes_present']}")
    if a.json:
